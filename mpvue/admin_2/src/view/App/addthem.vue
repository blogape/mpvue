...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (_this.form.title == "") {
        this.$message.error("主题标题为空，请正确填写");
      } else if (_this.form.describle == "") {
        this.$message.error("主题描述为空，请正确填写");
      }  else {
        this.axios
          .post(
            "http://192.168.1.52:8765/api/cms-service/recipeTopic/save?token=666666",
            {
              title: _this.form.title,
              description:  _this.form.describle,
              url: _this.form.url,
              coverimg: _this.inimages,
            }
          )
          .then(response => {
